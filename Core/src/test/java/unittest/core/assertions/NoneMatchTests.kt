...
class NoneMatchTests : LocalTestModule()
{
	...
	@Test
	fun `assert that noneMatch, on success, gives summarized results for the last element that was processed`()
	{
		val result = test {
	...
	@Test
	fun `assert that noneMatch, on failure, gives summarized results for the matched element in the collection`()
	{
		val result = test {
	...
	@Test
	fun `assert that noneMatch, on failure, gives a summary message after validating the failing elements`()
	{
		val result = test {
			assertThat(failureValues)
				.noneMatch { test(it) }
	...
	@Test
	fun `assert that noneMatch, on success, gives a summary message`()
	{
		val result = test {
			assertThat(successValues)
				.noneMatch { test(it) }
	...
	@Test
	fun `assert that noneMatch, on failure, by unexpected exception, throws the exception`()
	{
		val result = test {
			assertThat(successValues)
				.noneMatch { fail() }
	...
	@Test
	fun `assert that noneMatch, on success, only consumes the feed iterable once`()
	{
		var consumptions = 0
	...
	@Test
	fun `assert that noneMatch, on success, only consumes the feed sequence once`()
	{
		var consumptions = 0
