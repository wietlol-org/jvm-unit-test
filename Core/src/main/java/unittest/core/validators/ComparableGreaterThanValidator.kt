...
class ComparableGreaterThanValidator<T : Comparable<T>>(
...
) : Validator<T>, ValidatorHelper
{
	...
	private fun generateMessage(value: T, isValid: Boolean): String =
		if (isValid)
			"$messageIndent'isGreaterThan(base=${base.toJson()})' assertion succeeded"
		else
		{
			val valueDisplay = value.toJson()
			val baseDisplay = base.toJson()
			
			"$messageIndent'isGreaterThan(base=$baseDisplay)' assertion failed:\n" +
