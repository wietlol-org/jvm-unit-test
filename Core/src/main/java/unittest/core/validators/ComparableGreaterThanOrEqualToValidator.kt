...
class ComparableGreaterThanOrEqualToValidator<T : Comparable<T>>(
...
) : Validator<T>, ValidatorHelper
{
	...
	private fun generateMessage(value: T, isValid: Boolean): String =
		if (isValid)
			"$messageIndent'isGreaterThanOrEqualTo(base=${base.toJson()})' assertion succeeded"
		else
		{
			val valueDisplay = value.toJson()
			val baseDisplay = base.toJson()
			
			"$messageIndent'isGreaterThanOrEqualTo(base=$baseDisplay)' assertion failed:\n" +
