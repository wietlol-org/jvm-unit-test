...
class ComparableLessThanValidator<T : Comparable<T>>(
...
) : Validator<T>, ValidatorHelper
{
	...
	private fun generateMessage(value: T, isValid: Boolean): String =
		if (isValid)
			"$messageIndent'isLessThan(base=${base.toJson()})' assertion succeeded"
		else
		{
			val valueDisplay = value.toJson()
			val baseDisplay = base.toJson()
			
			"$messageIndent'isLessThan(base=$baseDisplay)' assertion failed:\n" +
