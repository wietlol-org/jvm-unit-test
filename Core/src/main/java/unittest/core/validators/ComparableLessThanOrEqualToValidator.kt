...
class ComparableLessThanOrEqualToValidator<T : Comparable<T>>(
...
) : Validator<T>, ValidatorHelper
{
	...
	private fun generateMessage(value: T, isValid: Boolean): String =
		if (isValid)
			"$messageIndent'isLessThanOrEqualTo(base=${base.toJson()})' assertion succeeded"
		else
		{
			val valueDisplay = value.toJson()
			val baseDisplay = base.toJson()
			
			"$messageIndent'isLessThanOrEqualTo(base=$baseDisplay)' assertion failed:\n" +
