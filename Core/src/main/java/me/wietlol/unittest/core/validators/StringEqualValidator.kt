...
) : Validator<CharSequence>, ValidatorHelper
{
	...
	override fun validate(value: CharSequence): Validation
	{
		...
		val isValid = valueText.equals(baseText, ignoreCase)
	...
	private fun generateMessage(valueText: String, isValid: Boolean, displayLength: Int): String =
		if (isValid)
			"$messageIndent'isEqualTo(base=${baseText.toDisplayString()}, ignoreCase=$ignoreCase)' assertion succeeded"
		else
		{
			...
			val offset = if (invalidIndex > halfDisplayLength)
				max(min(invalidIndex - halfDisplayLength, valueText.length - displayLength), 0)
			else
				0
			
			val valueDisplay = valueText.toDisplayString(displayLength, offset)
			val baseDisplay = baseText.toDisplayString(displayLength, offset)
			
			"$messageIndent'isEqualTo(base=${baseText.toDisplayString()}, ignoreCase=$ignoreCase)' assertion failed:\n" +
	...
	private fun findInvalidIndex(value: String, head: String, ignoreCase: Boolean): Int =
		(0..min(value.length, head.length))
			.asSequence()
			.filter { it == head.length || it == value.length || value[it].equals(head[it], ignoreCase).not() }
			.first()
}
